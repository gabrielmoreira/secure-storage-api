...
test('clearUserStorage removes only user-scoped values and preserves app-scoped values', async () => {
  ...
  const userProperty = defineSecureStorageProperty({
    namespace: 'auth',
    name: 'refreshToken',
  });
  const appProperty = defineSecureStorageProperty({
    namespace: 'core',
...
test('userPresence properties pass backend access options for reads and writes', async () => {
  ...
  const sensitiveCalls = calls.filter(
    (call) => call.method === 'setItem' || call.method === 'getItem',
  );
...
test('get persists async defaultValue only after storage miss', async () => {
  ...
  assert.deepEqual(await storage.get(property), { selectedAccountId: null });
  assert.deepEqual(await storage.get(property), { selectedAccountId: null });
...
test('get writes back normalized encoded values returned by codecs', async () => {
  ...
  const property = defineSecureStorageProperty({
    ...
    codec: {
      ...
      decode(encodedValue) {
        ...
        return {
          value: normalized,
          normalizedEncodedValue: normalized,
        };
      },
    },
  });
  ...
  assert.equal(await storage.get(property), 'MIXED-CASE');
